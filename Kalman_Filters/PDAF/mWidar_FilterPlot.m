...
%}
    ...
    for k = 1:n_k
        ...
        std_xv(k) = sqrt(P{k}(2, 2));
        std_xa(k) = sqrt(P{k}(3, 3));

        std_yp(k) = sqrt(P{k}(4, 4));
        std_yv(k) = sqrt(P{k}(5, 5));
    ...
    for k = 1:n_k
        ...
        innovCov = [S{k}(1, 1) S{k}(1, 2); S{k}(2, 1) S{k}(2, 2)];
        ...
        plot3(X(1, k), X(4, k), ones(length(GT), 1), 'ms', 'MarkerSize', 12, 'LineWidth', 1.2)
        plot3(GT(1, k), GT(4, k), ones(length(GT), 1), 'mx', 'MarkerSize', 10, 'LineWidth', 10)
