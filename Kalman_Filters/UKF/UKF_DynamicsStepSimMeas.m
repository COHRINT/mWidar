function [x_minus,y_minus,Pxx,Pyy,Pxy] = UKF_DynamicsStepSimMeas(x,P,f,Q,R,M,G)
...
alpha = 1e-1; % Large a -> SP more spread out
kappa = 1500; %Heavily Inflate s.t. weights are not negative
...
SP(:,1) = X_aug;
 ...
 for j = 1:2*n+1
     ...
     %[ChiBarX(1,j);ChiBarX(4,j)]

     [gamma(:,j),~] = SimulateImages(ChiBarX(:,j),M,G);

     gamma(:,j) = gamma(:,j) + Chir(:,j);
