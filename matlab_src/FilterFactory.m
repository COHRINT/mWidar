...
% See also FilterConfig, DA_Filter, KF_RBPF, HMM_RBPF, PDA_KF, GNN_KF
    ...
    switch upper(cfg.filter_name)
        ...
        % For multi-target filters, x0 must be a cell array {1 x N_t}
        % ==============================================================

        case 'KF_RBPF_MULTI'
        % KF_RBPF_multi(x0_cell, N_particles, F, Q, H, R, ...)
        % x0 must be {1 x N_t} cell of [N_x x 1] initial states
        % ==============================================================
            filt = KF_RBPF_multi(x0, cfg.N_particles, cfg.F, cfg.Q, cfg.H, cfg.R, ...
                                  'Debug', cfg.Debug, ...
                                  'DynamicPlot', cfg.DynamicPlot, ...
                                  'PD', cfg.PD, ...
                                  'PFA', cfg.PFA, ...
                                  'ESSThreshold', cfg.ESS_threshold, ...
        ...
        case 'PDA_PF_MULTI'
        % PDA_PF_multi(x0_cell, N_particles, F, Q, H, R, ...)
        % x0 must be {1 x N_t} cell of [N_x x 1] initial states
        % ==============================================================
            filt = PDA_PF_multi(x0, cfg.N_particles, cfg.F, cfg.Q, cfg.H, cfg.R, ...
                                 'Debug', cfg.Debug, ...
                                 'PD', cfg.PD, ...
                                 'PFA', cfg.PFA, ...
                                 'LambdaClutter', cfg.lambda_clutter, ...
                                 'ValidationSigma', cfg.ValidationSigma, ...
                                 'ESSThreshold', cfg.ESS_threshold, ...
                                 'PointlikelihoodImage', pointlikelihood_image);

        case 'HMM_RBPF_MULTI'
        % HMM_RBPF_multi(x0_cell, N_particles, A_transition, pointlikelihood_image, ...)
        % x0 must be {1 x N_t} cell of [2 x 1] initial positions
        % ==============================================================
            if isempty(A_transition) || isempty(pointlikelihood_image)
                error('FilterFactory:MissingLikelihood', ...
                    'HMM_RBPF_multi requires A_transition (4th arg) and pointlikelihood_image (5th arg).');
            end
            filt = HMM_RBPF_multi(x0, cfg.N_particles, A_transition, ...
                                   pointlikelihood_image, ...
                                   'Debug', cfg.Debug, ...
                                   'PD', cfg.PD, ...
                                   'PFA', cfg.PFA, ...
