...
//    std::system("python3 path/to/your/simulator.py");
    ...
    sem_t *truth_sem = VisionProcessor::initSemaphore("/object_sem", 0);
    void *image_ptr = VisionProcessor::initializeConnectionToSharedMemory("/image", V_IMAGE_SIZE);
    void *truth_ptr = VisionProcessor::initializeConnectionToSharedMemory("/objects", V_OBJECT_SIZE * 2);
    while (true)
    {
        // openCV stuff
        ...
        {
            std::cout << "ID: " << pair.first << " coords: " << pair.second << std::endl;
        }
